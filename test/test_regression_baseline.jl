...
# cost-wedge calibration and the default change n_strangers = 5.
@testset "Regression Baseline" begin
    ...
    # Match counts
    @test mean(tail.n_total_matches) ≈ 74.533333 atol=0.01

    # Outsourcing rate
    @test mean(tail.outsourcing_rate) ≈ 0.016507 atol=1e-4
    ...
    @test broker_r2 ≈ -0.003051 atol=1e-4
    @test agent_r2 ≈ -0.364889 atol=1e-4

    # Match output
    @test mean(filter(!isnan, tail.q_self_mean)) ≈ 1.561761 atol=1e-4

    # Broker state at end
    @test df.betweenness[end] ≈ 0.0 atol=1e-6
    @test df.roster_size[end] == 3
end
