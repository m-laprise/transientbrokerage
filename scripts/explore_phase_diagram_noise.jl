"""
    explore_phase_diagram_noise.jl

Phase diagram heatmaps over a (ρ, SNR) grid at d=8, η=0.05, complex geometry.
SNR = 1/σ_ε (signal bounded in [-1,1], so max amplitude = 1). Runs both
base model and staffing model for T=200 periods with 5 seeds per cell.

Output: data/figures/exploration/phasediagrams/noise_sweep/
Data:   data/sims/exploration/phasediagrams/data/noise_sweep/

Usage: julia --project --threads=auto scripts/explore_phase_diagram_noise.jl
       julia --project --threads=auto scripts/explore_phase_diagram_noise.jl --rerun
"""
...
const OUTDIR = joinpath(@__DIR__, "..", "data", "figures", "exploration", "phasediagrams", "noise_sweep")
const DATADIR = joinpath(@__DIR__, "..", "data", "sims", "exploration", "phasediagrams", "data", "noise_sweep")
mkpath(OUTDIR); mkpath(DATADIR)

const RERUN = "--rerun" in ARGS

const RHOS = 0.0:0.1:1.0 |> collect
# σ_ε values from high noise to low noise; displayed as SNR = 1/σ_ε
const SIGMA_EPS_VALS = [0.50, 0.40, 0.30, 0.25, 0.20, 0.15, 0.10, 0.05]
const SNR_VALS = [1.0 / s for s in SIGMA_EPS_VALS]  # 2, 2.5, 3.33, 4, 5, 6.67, 10, 20
const D = 8
const GEOM = :complex
const T = 200
const N_SEEDS = 5
...
cells = Matrix{Any}(undef, n_snr, n_rho)

function field_matrix(field::Symbol)
    M = Matrix{Float64}(undef, n_snr, n_rho)
    for ri in 1:n_rho, si in 1:n_snr
        M[si, ri] = getfield(cells[si, ri], field)
    end
    return M
end
...
function interpolate_grid(M, rhos, snrs; n_fine=200)
    rho_fine = range(first(rhos), last(rhos); length=n_fine)
    snr_fine = range(first(snrs), last(snrs); length=n_fine)
    Z = Matrix{Float64}(undef, n_fine, n_fine)
    for (jj, rf) in enumerate(rho_fine), (ii, sf) in enumerate(snr_fine)
        ri = searchsortedlast(rhos, rf)
        ri = clamp(ri, 1, length(rhos) - 1)
        si = searchsortedlast(snrs, sf)
        si = clamp(si, 1, length(snrs) - 1)
        tr = (rf - rhos[ri]) / (rhos[ri+1] - rhos[ri])
        ts = (sf - snrs[si]) / (snrs[si+1] - snrs[si])
        tr = clamp(tr, 0.0, 1.0)
        ts = clamp(ts, 0.0, 1.0)
        v00 = M[si, ri];     v10 = M[si+1, ri]
        v01 = M[si, ri+1];   v11 = M[si+1, ri+1]
        vs = [v00, v10, v01, v11]
        if any(isnan, vs)
            valid = filter(!isnan, vs)
            Z[ii, jj] = isempty(valid) ? NaN : mean(valid)
        else
            Z[ii, jj] = (1-tr)*(1-ts)*v00 + (1-tr)*ts*v10 + tr*(1-ts)*v01 + tr*ts*v11
        end
    end
    return rho_fine, snr_fine, Z
end
...
function make_phase_plot!(ax, cb_pos, M; colormap=Reverse(:RdBu), diverging=true,
    ...
    rho_fine, snr_fine, Z = interpolate_grid(M, collect(Float64, RHOS), SNR_VALS)
    levels = range(crange[1], crange[2]; length=n_levels + 1)

    cf = contourf!(ax, rho_fine, snr_fine, Z; colormap, levels, extendlow=:auto, extendhigh=:auto)
    if zero_contour
        contour!(ax, rho_fine, snr_fine, Z; levels=[0.0], color=:black, linewidth=1.5)
    end
    contour!(ax, rho_fine, snr_fine, Z; levels=collect(levels), color=(:gray30, 0.4), linewidth=0.5)

    ax.xticks = 0.0:0.2:1.0
    ax.yticks = [2, 4, 6, 8, 10, 15, 20]
    ax.xlabel = "ρ"
    ax.ylabel = "SNR (1/σ_ε)"
...
println("Phase diagram (noise sweep): $(n_rho) ρ × $(n_snr) SNR, d=$D, η=0.05, $GEOM geometry, $N_SEEDS seeds, T=$T")
println("  σ_ε = $SIGMA_EPS_VALS  →  SNR = $(round.(SNR_VALS, digits=1))")
RERUN && println("  --rerun: forcing re-simulation")
println()

for (ri, rho) in enumerate(RHOS)
    for (si, sigma_eps) in enumerate(SIGMA_EPS_VALS)
        tag = "rho$(lpad(round(Int, rho*100), 3, '0'))_seps$(lpad(round(Int, sigma_eps*100), 2, '0'))"
        base_file = joinpath(DATADIR, "$(tag)_base.jld2")
        staff_file = joinpath(DATADIR, "$(tag)_staffing.jld2")

        print("  ρ=$rho σ_ε=$sigma_eps (SNR=$(round(1/sigma_eps, digits=1))) ...")

        if !RERUN && isfile(base_file)
            mdfs_base = JLD2.load(base_file, "mdfs")
        else
            mdfs_base = run_ensemble(; base_params_kwargs=(d=D, rho=rho, sigma_eps=sigma_eps,
                                                            firm_geometry=GEOM),
                                       T=T, n_seeds=N_SEEDS)
            N_W = default_params(; d=D, rho=rho, sigma_eps=sigma_eps, firm_geometry=GEOM).N_W
        ...
        if !RERUN && isfile(staff_file)
            ...
            mdfs_staff = run_ensemble(; base_params_kwargs=(d=D, rho=rho, sigma_eps=sigma_eps,
                                                             firm_geometry=GEOM, enable_staffing=true),
                                        T=T, n_seeds=N_SEEDS)
            N_W = default_params(; d=D, rho=rho, sigma_eps=sigma_eps, firm_geometry=GEOM, enable_staffing=true).N_W
            JLD2.save(staff_file, "mdfs", mdfs_staff, "N_W", N_W)
        end

        cells[si, ri] = extract_cell(mdfs_base, mdfs_staff)
        println(" done")
    end
end
...
Label(fig1[0, 1:6],
      "Phase diagrams: How match noise shapes broker advantage and capture";
      fontsize=16, font=:bold, halign=:center, tellwidth=false)
Label(fig1[1, 1:6],
      "Complex geometry, d=$D, η=0.05, T=$T, steady-state means (t=$(first(WINDOW))-$(last(WINDOW))) of ensemble means over $N_SEEDS seeds.";
...
footer_lines = join([
    ...
    "SNR = 1/σ_ε where σ_ε is the match output noise SD. Higher SNR = cleaner signal, easier to learn.",
...
Label(fig3[0, 1:4],
      ...
      fontsize=16, font=:bold, halign=:center, tellwidth=false)
Label(fig3[1, 1:4],
      "Complex geometry, d=$D, η=0.05, T=$T, $N_SEEDS seeds. Shares from cumulative sums over all $T periods.";
      fontsize=11, halign=:center, tellwidth=false, color=:gray30)
